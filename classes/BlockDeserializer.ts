    ...
    try {
        switch(data.type) {
            case "CCodeBlockAssignment":
                return await CCodeBlockAssignment.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockValue":
                return await CCodeBlockValue.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockArrayInit":
                return await CCodeBlockArrayInit.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockBreak":
                return await CCodeBlockBreak.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockGetVariableValue":
                return await CCodeBlockGetVariableValue.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockIfStatement":
                return await CCodeBlockIfStatement.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockLogic":
                return await CCodeBlockLogic.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockLogicNot":
                return await CCodeBlockLogicNot.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockMath":
            ...
            case "CCodeBlockPrint":
                ...
                return await CCodeBlockPrint.deserialize(
                    data, 
                    onDrop, 
                    [],       // Пустой массив для go
                    () => {}, // Заглушка для gso
                    onPickUp
                );
            case "CCodeBlockWhile":
                return await CCodeBlockWhile.deserialize(data, onDrop, onPickUp);
            case "CCodeBlockWrapper":
                // Исправление для CCodeBlockWrapper
                return await CCodeBlockWrapper.deserialize(data) as unknown as ICodeBlock;
            default:
